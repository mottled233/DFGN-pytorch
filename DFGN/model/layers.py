...
class BaselinePredictionLayer(nn.Module):
    def __init__(self, config, q_dim):
        ...
        self.type_linear = nn.Linear(self.input_dim, 3)   # yes/no/ans
    ...
    def forward(self, batch, context_input, query_vec, entity_state, packing_mask=None, return_yp=False):
        ...
        sp_state = all_mapping.unsqueeze(3) * input_state.unsqueeze(2)  # N x sent x 512 x 300 不明白512和 sent为什么会这样换位置

        sp_state = sp_state.max(1)[0] # 取每个sent的最大的一个值
        ...
        if not return_yp:
            return start_logits, end_logits, sp_logits.squeeze(2), type_logits, None

        # 找结束位置用的开始和结束位置概率之和
        # (batch, 512, 1) + (batch, 1, 512) -> (512, 512)
        outer = start_logits[:, :, None] + end_logits[:, None] # None等于unsqueeze
        ...
        return start_logits, end_logits, sp_logits.squeeze(2), type_logits, None, start_position, end_position
